...
// Функция для вывода отформатированного объекта
...
public:
    ...
    // Метод для получения всех карт (добавлен для адаптера)
...
// Адаптер класса
class DeckAdapterClass : public Deck, public IFormattable {
public:
    DeckAdapterClass(int numDecks = 4, int cardsPerDeck = 52) : Deck(numDecks* cardsPerDeck) {}

    // Реализация метода format() из интерфейса IFormattable
    std::string format() const override {
        std::string formatted = "Колода содержит следующие карты:\n";
...
// Адаптер объекта
class DeckAdapterObject : public IFormattable {
private:
    const Deck& deck_;

public:
    DeckAdapterObject(const Deck& deck) : deck_(deck) {}

    // Реализация метода format() из интерфейса IFormattable
    std::string format() const override {
        std::string formatted = "Колода содержит следующие карты:\n";
        const auto& cards = deck_.getCards();
        for (const auto& card : cards) {
            formatted += card.toString() + "\n";
        }
        return formatted;
...
int main() {
    ...
    // Создание адаптера класса
    DeckAdapterClass adapterClass(1, 52); // 1 колода по 52 карты
    prettyPrint(adapterClass);

    // Создание колоды и адаптера объекта
    Deck anotherDeck(52);
    DeckAdapterObject adapterObject(anotherDeck);
    prettyPrint(adapterObject);
